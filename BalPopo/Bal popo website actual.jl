...
route("/photos/:fname") do
  fname = String(params(:fname)) |> strip
  # prevent path traversal
  root = abspath(cd_path*"BalPopo/photos")
  path = abspath(joinpath(root, replace(fname, "%20" => " ")))
  ...
  data = read(path)
  return HTTP.Response(200, ["Content-Type" => mime, "Content-Length" => string(length(data))], data)
end
